...
fn convert_le_to_be(le: u256) -> ByteArray {
    ...
    println!("Keccak (le hex): {:?}", le);

    if le.len() < 64 {
        le = pad_to_32_bytes(le);
    }
    ...
    while i < rev_le.len() {
        ...
        let first_byte = rev_le[i];
        let second_byte = rev_le[i + 1];

        two_bytes.append_byte(first_byte);
        two_bytes.append_byte(second_byte);

        let rev: ByteArray = two_bytes.rev();
        be_ba.append(@rev);
        i += 2;
    };
    be_ba
...
fn pad_to_32_bytes(ba_in: ByteArray) -> ByteArray {
...
fn decimal_to_byte_array(mut n: u256) -> ByteArray {
    ...
    if hex_ba_n.len() < 64 {
        hex_ba_n = pad_to_32_bytes(hex_ba_n);
    }
    let mut i = 0;
